...
@objc public class LoginService: NSObject {
    ...
    /// Controller which would is used to present web view.
    ...
    public func performAuthRequest(_ authRequest: AuthRequest, completion:@escaping (AuthResult) -> Void) {
        ...
        let authHelper = OAuthHelper.init(urlRequest: request, clientSecret: authRequest.clientSecret) { result in
        ...
        self.presentingViewController.show(navVC, sender: nil)
    }
    ...
    func createURLRequest(_ authRequest: AuthRequest) -> URLRequest {
        ...
        pathComponents!.queryItems = [URLQueryItem.init(name: "client_id", value: authRequest.clientID)]
